...
BCH`WeylQuantization	:= BCH`Private`WeylQuantization
ParalleEvaluateMatrix   := BCH`Private`ParalleEvaluateMatrix
...
(*ParallelRavelToMatrix[n_Integer, bch_] :=bch[[n]][[1]]*Fold[Parallelize@Dot, "Iden", StringSplit @@ {bch[[n]][[2]], ""}]*)
Decode2Matrix[A_,B_,bch_]:=Decode2Matrix[A,B,bch]=Module[{i,decode,mat,SUM, eye},
	decode = Table[RavelToMatrix[i,bch],{i,1,Length[bch]}];
  (*
  Print@Sum[decode[[i]],{i,1,Lenght[decoede]}];
  Exit[];
  *)
	If[Length@Kernels[]==0, SUM=Sum,SUM=ParallelSum];
  eye = IdentityMatrix[Dim];
...
WeylQuantization[Ham_] := Module[{coeff, k,i,j, comb, res, dimi, dimj, symbol},
  coeff = CoefficientList[Ham, {p, q}];
  {dimi, dimj} = Dimensions[coeff];
  Sum[
   If[coeff[[i, j]] != 0,
    symbol = ConstantArray[p, i - 1]~Join~ConstantArray[q, j - 1];
    comb = Permutations[symbol];
    res = Sum[Fold[Dot, comb[[k]]], {k, 1, Length@comb}]/Length[comb]*coeff[[i, j]],  0]
    , {i, 1, dimi}, {j, 1, dimj}]
   ]
ParalleEvaluateMatrix[Ham_, qmat_, pmat_]:=ParallelSum[Ham[[i]] /. {q -> qmat, p -> pmat}, {i, 1, Length[Ham]}]
